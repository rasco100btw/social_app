-- Add trigger to update message status
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    NEW.status := 'sent';
  -- When message is read, update status
  ELSIF TG_OP = 'UPDATE' AND NEW.status = 'read' THEN
    ...
    SELECT
      ...
      'message_read',
      'Message read',
      'Your message has been read by ' || (SELECT name FROM profiles WHERE id = OLD.receiver_id),
      '/messages?recipient=' || OLD.receiver_id
    WHERE OLD.status != 'read';
  END IF;
